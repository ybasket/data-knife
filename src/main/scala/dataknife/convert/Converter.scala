...
trait Converter[In <: Format: ValueOf, Out <: Format: ValueOf, Config] {
  ...
  def convertByteToByte(
      config: Config,
      inputOptions: inputFormat.InputOptions,
      outputOptions: outputFormat.OutputOptions
  ): Pipe[IO, Byte, Byte] =
    _.through(inputFormat.read(inputOptions))
      .through(convert(config))
      .through(outputFormat.write(outputOptions))
}
